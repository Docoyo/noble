...
Hci.prototype.setScanParameters = function(parameters) {
  ...
  if (parameters && parameters.interval) {
    var scanInterval = Math.round(parameters.interval / 1.6);
    console.log('Scan interval --> Scan interval setting', parameters.interval, scanInterval);
  } else {
    var scanInterval = 0x0010;
  }

  // scan window
  if (parameters && parameters.window) {
    var scanWindow = Math.round(parameters.window / 1.6);
    console.log('Scan window --> Scan window setting', parameters.window, scanWindow);
  } else {
    var scanWindow = 0x0010;
  }

  // data
  cmd.writeUInt8(parameters && parameters.active ? 0x01 : 0x00, 4); // type: 0 -> passive, 1 -> active
  cmd.writeUInt16LE(scanInterval, 5); // interval, ms * 1.6
  cmd.writeUInt16LE(scanWindow, 7); // window, ms * 1.6
