...
Noble.prototype.startScanning = function(serviceUuids, allowDuplicates, callback) {
  var scan = function(state) {
    if (state !== 'poweredOn') {
      ...
      this._bindings.startScanning(serviceUuids, allowDuplicates, this.scanParams);
    }
  };
...
/*
* Supported properties in parameters:
* - active: true/false
* - interval: Scan interval in ms
* - window: Scan window in ms
*/
Noble.prototype.setScanParameters = function(parameters) {
  this.scanParams = parameters;
  console.log('Set scan parameters to', parameters);
}
